...
protected:
	...
	static constexpr volatile uint8_t* gport() {
		if constexpr (port == Gpio::Port::B) return &PORTB;
		if constexpr (port == Gpio::Port::C) return &PORTC;
		if constexpr (port == Gpio::Port::D) return &PORTD;
		return nullptr;
	}
	static constexpr volatile uint8_t* gpin() {
		if constexpr (port == Gpio::Port::B) return &PINB;
		if constexpr (port == Gpio::Port::C) return &PINC;
		if constexpr (port == Gpio::Port::D) return &PIND;
		return nullptr;
	}
	static constexpr volatile uint8_t* gddr() {
		if constexpr (port == Gpio::Port::B) return &DDRB;
		if constexpr (port == Gpio::Port::C) return &DDRC;
		if constexpr (port == Gpio::Port::D) return &DDRD;
		return nullptr;
	}
public:
	// GpioOutput
